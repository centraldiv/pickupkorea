...
import SinglePFOrderItems from "./SinglePFOrderItems";

const SinglePFOrderContainer = ({ orderId }: { orderId: string }) => {
  const { data, isLoading, isError } = useSinglePFOrder(orderId);
  ...
  if (data) {
    return (
      <section className="flex flex-col gap-4 max-w-7xl mx-auto w-full py-6 min-h-screen px-4">
        ...
        <SinglePFOrderItems orderId={orderId} />
      </section>
    );
  }
};

export default SinglePFOrderContainer;
